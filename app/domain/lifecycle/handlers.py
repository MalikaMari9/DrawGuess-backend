...
async def _build_snapshot(app, room_code: str, mode: Mode) -> OutRoomSnapshot:
    """
    ...
    # Never expose secret_word in snapshots
    if "secret_word" in round_cfg:
...
async def handle_join(*, app, room_code: str, pid: Optional[str], msg: InJoin) -> Result:
    ...
    snapshot = await _build_snapshot(app, room_code, header.mode)
...
async def handle_snapshot(*, app, room_code: str, pid: Optional[str], msg: InSnapshot) -> Result:
    ...
    snap = await _build_snapshot(app, room_code, header.mode)
